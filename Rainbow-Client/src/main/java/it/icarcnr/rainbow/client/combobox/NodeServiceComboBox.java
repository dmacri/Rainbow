...
public class NodeServiceComboBox extends ComboBox {
	...
	public NodeServiceComboBox(UrlParam[] baseParams, LoadOption loadOption, Boolean startLoad) {
		super();
		if(startLoad!=null){
	...
	private void initComboBox() {
		...
		setMinChars(3);  
		setFieldLabel(iComboBox.NodeServiceComboBox_Node()); //$NON-NLS-1$
//		setLabelStyle("font-size:12px");
		setDisplayField("description");  //$NON-NLS-1$
		setMode(ComboBox.LOCAL);  
		setTriggerAction(ComboBox.ALL);  
		setEmptyText(iComboBox.NodeServiceComboBox_Enter_nodoService_type());   //$NON-NLS-1$
		setLoadingText(iComboBox.NodeServiceComboBox_Searching());   //$NON-NLS-1$
		setTypeAhead(true);  
	...
	private Store getCBStore() {
		HttpProxy dataProxyLoad = new HttpProxy(IActionPathConstants.COMBO_BOX_NODE_SERVICE_ACTION_PATH);       
