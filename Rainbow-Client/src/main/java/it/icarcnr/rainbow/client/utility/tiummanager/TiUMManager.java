...
/**
 * @author Domenico Minnici
 *
 */
public class TiUMManager extends BasePanel{
	...
	private FormPanel makeFomPanel() {
		...
		executeButton = new Button(iUtility.TiUMManager_OK());
		...
		multiFieldPanel.addToRow(executeButton, 70);
	...
	private Store getStore() {
		HttpProxy dataProxyLoad = new HttpProxy(IActionPathConstants.TIUMMANAGER_ACTION_PATH);       
				...
				if(records!=null && records.length > 0) {
					...
					if (reachedMaxNumberOfProcess) {
						...
						if(waitExecution.contains(("No Results!"))){
							...
							TiUMManagerPanel tiUMManagerPanel = new TiUMManagerPanel();
							tiUMManagerPanel.addParameter("waitExecution", new JSONString(waitExecution));
							tiUMManagerPanel.addParameter("telNumber", new JSONString(telNumber));
							tiUMManagerPanel.addParameter("numberNoCheck", JSONBoolean.getInstance(numberNoCheck));
							tiUMManagerPanel.addParameter("nameJob",  new JSONString(nameJob));
							
							tiUMManagerPanel.setWindowParent(window);
							tiUMManagerPanel.setPanelParent(TiUMManager.this);
							tiUMManagerPanel.init();
							window.add(tiUMManagerPanel);
							window.show();
						}
					}
