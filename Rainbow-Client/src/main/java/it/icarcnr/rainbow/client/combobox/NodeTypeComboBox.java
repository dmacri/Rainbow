...
public class NodeTypeComboBox extends ComboBox {
	...
	public NodeTypeComboBox(UrlParam[] baseParams, LoadOption loadOption, Boolean startLoad) {
		super();
		if(startLoad!=null){
	...
	private void initComboBox() {
		...
		setMinChars(3);  
		setFieldLabel(iComboBox.NodeTypeComboBox_Node_Type()); //$NON-NLS-1$
//		setLabelStyle("font-size:12px");
		setDisplayField("description");  //$NON-NLS-1$
		setMode(ComboBox.LOCAL);  
		setTriggerAction(ComboBox.ALL);  
		setEmptyText(iComboBox.NodeTypeComboBox_Enter_node_type());   //$NON-NLS-1$
		setLoadingText(iComboBox.NodeTypeComboBox_Searching());   //$NON-NLS-1$
		setTypeAhead(true);  
	...
	private Store getCBStore() {
		HttpProxy dataProxyLoad = new HttpProxy(IActionPathConstants.NODE_TYPE_COMBO_BOX_ACTION_PATH);       
