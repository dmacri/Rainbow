package it.icarcnr.business.quartz.impl;

import it.icarcnr.business.utility.impl.UtilityBL;
        ...
        try {
	        ...
	        for (Cronjob cronjob : listcronjob) {
				...
				if (!file.exists()) {
					...
					JobDetail jDetail = createJobDetail(cronjob.getNamejob(), cronjob.getIdUser(), cronjob.getUtility(), cronjob.getOutput(), command, cronjob.getDate(), Boolean.FALSE);
