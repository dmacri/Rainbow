...
public class CheckServicesStatusPermissionAction extends Action {
	
	private static final Log log = LogFactory.getLog(CheckServicesStatusPermissionAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			Boolean tackingChargePermission = iSecurityBL.hasPermission(userLoginBean.getId(), IActionConstants.TAKE_CARE_ACTION, networkId, functionId);
			Boolean logViewPermission = iSecurityBL.hasPermission(userLoginBean.getId(), IActionConstants.LOG_VIEW_ACTION, networkId, functionId);
			Boolean wayViewPermission = iSecurityBL.hasPermission(userLoginBean.getId(), IActionConstants.WAY_VIEW_ACTION, networkId, functionId);

			JSONObject result = new JSONObject();
			JSONArray records = new JSONArray();
			JSONObject record = new JSONObject();

			record.put("tackingChargePermission", tackingChargePermission);		
			record.put("logViewPermission", logViewPermission);
			record.put("wayViewPermission", wayViewPermission);
