...
public class CheckDownloadManualIpccPermissionAction extends Action {
	
private static final Log log = LogFactory.getLog(CheckDownloadManualIpccPermissionAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			Boolean menuManualPermission = iSecurityBL.hasPermission(userLoginBean.getId(), IActionConstants.DOWNLOAD_IPCC_MANUAL, networkId, functionId);
			
			JSONObject result = new JSONObject();
			JSONArray records = new JSONArray();
			JSONObject record = new JSONObject();

			record.put("menuManualPermission", menuManualPermission);	
